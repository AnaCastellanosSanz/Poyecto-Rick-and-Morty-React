...
function ListPerson ({ list }) {
    ...
    const personLi = list.map((oneElement) => {
     ...
     return <Person oneElement={oneElement} key={oneElement.id}/>; //Aqui es donde envío la props cada vez que llame a Person (De esta manera obtengo los datos de oneElement. La Key viene porque en la consola aparece un error de que se necesita poner una key para identificar cada elemento, se tiene que poner al hijo directo del map.
